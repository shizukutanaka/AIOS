"""Pass 27 regression tests: Go port version mismatch (1.4.0→1.7.0); stale test count."""
...
class TestGoPortVersion(unittest.TestCase):
    ...
    def test_version_var_is_not_stale(self):
        """The `version` package-level var must be 1.7.0, not the old 1.4.0 value."""
        src = self._go_src()
        m = re.search(r'version\s*=\s*"([^"]+)"', src)
        self.assertIsNotNone(m, "version variable not found in main.go")
        self.assertEqual(
            m.group(1), "1.7.0",
            f"Go port version should be 1.7.0, got '{m.group(1)}'. "
    ...
    def test_cmdinfo_json_version_matches(self):
        """cmdInfo JSON output must advertise 1.7.0."""
        src = self._go_src()
        ...
        self.assertIn(
            '"version":         "1.7.0"',
            body,
            "cmdInfo JSON block must set version to 1.7.0",
        )
