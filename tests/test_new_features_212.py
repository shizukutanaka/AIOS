...
# Read from sum.golang.org, entry by entry, and confirmed to match what
# proxy.golang.org served for the same module. This is the authority go.sum
# exists to record, so it is pinned here: the repository's go.sum is only
# trustworthy insofar as it still says exactly this.
CHECKSUM_DB_RECORD = {
...
class TestThisRepositoryIsClean(unittest.TestCase):
    ...
    def test_go_sum_is_present_again(self):
        # This test used to assert the file was absent, and asked in its own
        # message: "was it generated by `go mod download`, or written by
        # hand?" The answer is neither — every entry was read from
        # sum.golang.org and matched against what the module proxy served.
        # The question was the right one, so the assertion below answers it
        # rather than merely permitting the file back.
        self.assertTrue(Path("go-port/go.sum").exists())

    def test_every_entry_matches_the_checksum_database(self):
        # Pinned to the values sum.golang.org returned. A future edit that
        # changes a hash fails here unless whoever makes it re-verifies
        # against the checksum database and updates this list too.
        recorded = {
            line.rsplit(" ", 1)[0]: line.rsplit(" ", 1)[1]
            for line in Path("go-port/go.sum").read_text().splitlines()
            if line.strip()
        }
        for entry, digest in CHECKSUM_DB_RECORD.items():
            self.assertIn(entry, recorded, f"{entry} missing from go.sum")
            self.assertEqual(recorded[entry], digest,
                             f"{entry} does not match the checksum database")

    def test_go_sum_carries_no_unverified_entries(self):
        # The reverse direction: an entry nobody checked must not ride along.
        recorded = {line.rsplit(" ", 1)[0]
                    for line in Path("go-port/go.sum").read_text().splitlines()
                    if line.strip()}
        self.assertEqual(recorded - set(CHECKSUM_DB_RECORD), set())

    def test_the_reasoning_is_recorded_where_a_maintainer_will_look(self):
        # The history has to survive, or the next person reads a normal-looking
        # go.sum and never learns why it is pinned in a test.
        readme = Path("go-port/README.md").read_text()
        self.assertIn("sum.golang.org", readme)
...
class TestRemovalRestoresVerification(unittest.TestCase):
    ...
    def test_this_repo_has_no_unrecorded_modules(self):
        # Was the inverse assertion while go.sum was absent. Now that every
        # required module has a verified entry, nothing should be outstanding.
        self.assertEqual(missing_go_sum_entries(Path(".")), [])
