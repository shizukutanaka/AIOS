...
>
> 実測値（2026-09、本パスで計測）: **4,041 tests / 289 test files**、
...
| # | 2026-07 の主張 | 現況 | 根拠 |
|---|---|---|---|
| 1 | バージョン文字列が **20 ファイル**に分散 | **過大評価だった。実際に編集が要るのは 3 ファイル** | 実測: 値としてハードコードするのは `core/constants.py:142`・`pyproject.toml:7`・`go-port/cmd/aictl/main.go:26` のみ。他の 5 件は「v1.7.0 機能」等の**散文中の言及**で、バンプ時に編集不要 |
...
4. **`aictl demo` の bind アドレスは今日まで変更不能だった** — 127.0.0.1 固定で、
   コンテナからは到達不能。既定は安全側のまま `--host` を追加して解決（本パス）。

## 4. 改善案の現況

本文の提案 10 件のうち: **#8（CHANGELOG 動的参照）と #10（リリース自動化）は完了**
（それぞれ pass 207 / #41）。**#2（CI 追加）は権限待ちで不変**。#1 は上記のとおり
前提の見積り自体が過大だったため、残作業は「`go-port/main.go` のバージョンを
gate の Version フェーズが検証していない」1 点に縮小する（現在 gate は
`constants.py` と `pyproject.toml` の一致のみ検証）。#3–#7, #9 は未着手のまま有効。

**次に効果が大きい 3 件**（本追補時点の判断）:

1. **CI**（提案 #2）— 権限が付与され次第。ローカル gate は強固だが、サーバ側検証は
   依然ゼロ。
2. **`go-port` バージョン検証** — gate の Version フェーズに 3 つ目の情報源として
   `main.go` を追加。小さく、短所1 の残りを閉じる。
